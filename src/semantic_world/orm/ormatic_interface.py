...
from typing import get_type_hints, get_origin, get_args, List, Union
...
class RosMessageMixin:
    __ros_pkg__ = "semantic_digital_twin_msgs.msg"

    def get_ros_message(self):
        # NOTE: this keeps your existing behavior of stripping "DAO" for instantiation.
        type_name = self.__tablename__
        msg_cls = getattr(import_module(self.__ros_pkg__), type_name)
        msg = msg_cls()

        mapper = sa_inspect(self.__class__)

        # Copy columns except id and *_id
        for col in mapper.column_attrs:
            name = col.key
            if name == "id" or name.endswith("_id"):
                continue
            setattr(msg, name, getattr(self, name))

        # Recurse relationships
        for rel in mapper.relationships:
            ...
            if value is None:
                continue
            if rel.uselist:
                setattr(msg, name, [child.get_ros_message() for child in value])
            else:
                setattr(msg, name, value.get_ros_message())

        return msg

    # ---------------------------
    # NEW: write a .msg file
    # ---------------------------
    @classmethod
    def to_ros_message_file(cls, file_path: str) -> None:
        """
        Generate a ROS .msg definition for this DAO and write it to file_path.

        Rules:
        - Includes ALL mapped columns (id, *_id, etc.)
        - Relationships become nested message fields using DAO names:
          'semantic_digital_twin_msgs/<RelatedDAO>' (arrays for uselist=True)
        - Primitive mappings:
            int  -> int32
            float-> float64
            str  -> string
        ...
        file_path = os.path.join(file_path, f"{cls.__tablename__}.msg")

        mapper = sa_inspect(cls)

        # Prefer annotations to refine JSON/array types like List[float]
        hints = get_type_hints(cls, include_extras=True)

        def py_anno_for(attr_name):
            return hints.get(attr_name)

        def unwrap_optional(t):
            if get_origin(t) is Union:
                args = tuple(a for a in get_args(t) if a is not type(None))  # noqa: E721
                if len(args) == 1:
                    return args[0]
            return t

        def ros_scalar_from_py(py_t) -> str:
            # Map python primitives to ROS scalars
            if py_t is int:
                return "int32"
            if py_t is float:
                return "float64"
        ...
        def ros_type_for_column(attr_name, col_prop) -> str:
            anno = py_anno_for(attr_name)
            # If annotated with Mapped[T], peel it
            if anno is not None and get_origin(anno) is not None:
                # Handle Mapped[...] wrapper
                if getattr(get_origin(anno), "__name__", "") == "Mapped":
                    anno = get_args(anno)[0]
            if anno is not None:
                anno = unwrap_optional(anno)
                if get_origin(anno) in (list, List):
                    inner = unwrap_optional(get_args(anno)[0])
                    return f"{ros_scalar_from_py(inner)}[]"
                return ros_scalar_from_py(anno)

            # Fall back to SQLAlchemy column type
            try:
                ...
                if isinstance(col.type, SAJSON):
                    # Unknown element type → string array as a conservative default
                    return "string[]"
            except Exception:
                pass
            return "string"
        ...
        # 1) Columns (in mapping order)
        for col_prop in mapper.column_attrs:
            name = col_prop.key
            ros_t = ros_type_for_column(name, col_prop)
            lines.append(f"{ros_t} {name}")

        # 2) Relationships
        for rel in mapper.relationships:
            field_name = rel.key
            target_cls = rel.mapper.class_
            target_msg = f"semantic_digital_twin_msgs/msgs/{getattr(target_cls, '__tablename__', target_cls.__name__)}"
            if rel.uselist:
                target_msg = f"{target_msg}[]"
            lines.append(f"{target_msg} {field_name}")
...
class Base(DeclarativeBase, RosMessageMixin):
    type_mappings = {
        Type: TypeType,
    }
