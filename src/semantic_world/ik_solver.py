...
class InverseKinematicsSolver:
    ...
    def _solve_iteratively(
        self, qp_problem: QPProblem, solver_state: SolverState, dt: float, max_iterations: int
    ) -> np.ndarray:
        """
    ...
    def _solve_qp_step(
        ...
        (xstar, fval, exitflag, info) = daqp.solve(
            qp_matrices.H, qp_matrices.g, qp_matrices.A, qp_matrices.u, qp_matrices.l, sense
        )
...
class QPProblem:
    ...
    def __post_init__(self):
        ...
        self.active_dofs, self.passive_dofs, self.active_symbols, self.passive_symbols = (
            self._extract_dofs()
        )
    ...
    def _extract_dofs(
        self,
    ) -> Tuple[list[DegreeOfFreedom], list[DegreeOfFreedom], list[cas.Symbol], list[cas.Symbol]]:
        """
        ...
        root_to_common_link, common_link_to_tip = self.world.compute_split_chain_of_connections(
            self.root, self.tip
        )
    ...
    def _setup_weights(self):
        ...
        dof_weights = [
            0.001 * (1.0 / min(1.0, dof.upper_limits.velocity)) ** 2 for dof in self.active_dofs
        ]
