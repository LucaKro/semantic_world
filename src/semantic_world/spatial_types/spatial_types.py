...
class TransformationMatrix(Symbol_, ReferenceFrameMixin):
    ...
    @classmethod
    def from_xyz_rpy(
        cls, x=0, y=0, z=0, roll=0, pitch=0, yaw=0, reference_frame=None, child_frame=None
    ):
        p = Point3(x, y, z)
    ...
    def __deepcopy__(self, memo) -> TransformationMatrix:
        ...
        return TransformationMatrix(
            deepcopy(self.s), reference_frame=self.reference_frame, child_frame=self.child_frame
        )
...
class RotationMatrix(Symbol_, ReferenceFrameMixin):
    ...
    def x_vector(self):
        return Vector3(
            x=self[0, 0], y=self[1, 0], z=self[2, 0], reference_frame=self.reference_frame
        )

    def y_vector(self):
        return Vector3(
            x=self[0, 1], y=self[1, 1], z=self[2, 1], reference_frame=self.reference_frame
        )

    def z_vector(self):
        return Vector3(
            x=self[0, 2], y=self[1, 2], z=self[2, 2], reference_frame=self.reference_frame
        )
    ...
    def to_angle(self, hint=None):
        ...
        if hint is not None:
            return normalize_angle(if_greater_zero(hint(axis), if_result=angle, else_result=-angle))
        else:
            return angle
    ...
    def to_rpy(self):
        ...
        if0 = cy - _EPS
        ax = if_greater_zero(if0, atan2(self[k, j], self[k, k]), atan2(-self[j, k], self[j, j]))
...
class Quaternion(Symbol_, ReferenceFrameMixin):
    ...
    def conjugate(self):
        return Quaternion(
            x=-self[0], y=-self[1], z=-self[2], w=self[3], reference_frame=self.reference_frame
        )
...
all_expressions = Union[
    Symbol_, Symbol, Expression, Point3, Vector3, RotationMatrix, TransformationMatrix, Quaternion
]
...
def quaternion_slerp(q1, q2, t):
    ...
    return Quaternion.from_iterable(
        if_greater_eq_zero(
            if1, q1, if_greater_zero(if2, 0.5 * q1 + 0.5 * q2, ratio_a * q1 + ratio_b * q2)
        )
    )
...
def save_division(nominator, denominator, if_nan=None):
    if if_nan is None:
        if isinstance(nominator, Vector3):
            ...
            if_nan = 0
    save_denominator = if_eq_zero(condition=denominator, if_result=1, else_result=denominator)
    return if_eq_zero(denominator, if_result=if_nan, else_result=nominator / save_denominator)
...
def project_to_cone(frame_V_current, frame_V_cone_axis, cone_theta):
    ...
    project_on_cone_boundary = if_less(
        ...
        else_result=s
        * (cos(cone_theta) * frame_V_cone_axis_norm + sin(cone_theta) * (v_perp / norm_v_perp)),
    )
...
def angle_between_vector(v1, v2):
    v1 = v1[:3]
    v2 = v2[:3]
    return acos(limit(dot(v1.T, v2) / (norm(v1) * norm(v2)), lower_limit=-1, upper_limit=1))
...
def solve_for(
    expression, target_value, start_value=0.0001, max_tries=10000, eps=1e-10, max_step=50
):
...
def replace_with_three_logic(expr):
    ...
    if op == ca.OP_AND:
        return logic_and3(
            replace_with_three_logic(cas_expr.dep(0)), replace_with_three_logic(cas_expr.dep(1))
        )
    if op == ca.OP_OR:
        return logic_or3(
            replace_with_three_logic(cas_expr.dep(0)), replace_with_three_logic(cas_expr.dep(1))
