from ..views import Cabinet, Chair, Container, Door, Drawer, Fridge, Handle
...
def conditions_192830222825406162866735339839935932811(case) -> bool:
    def conditions_for_world_views_of_type_chair(case: World) -> bool:
        """Get conditions on whether it's possible to conclude a value for World.views  of type Chair."""
        return True

    return conditions_for_world_views_of_type_chair(case)


def conclusion_192830222825406162866735339839935932811(case) -> List[Chair]:
    def world_views_of_type_chair(case: World) -> List[Chair]:
        """Get possible value(s) for World.views  of type Chair."""
        return [Chair(body=b) for b in case.bodies if 'stool' in b.name.name.lower()]

    return world_views_of_type_chair(case)
