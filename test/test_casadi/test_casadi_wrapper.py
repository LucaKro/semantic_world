...
class TestRotationMatrix:
    ...
    def test_compilation(self):
        ...
        compiled_rotation = cas.compile_and_execute(
            lambda angle: cas.RotationMatrix.from_axis_angle(cas.Vector3.Z(), angle), [np.pi / 4]
        )
...
class TestPoint3:
    ...
    def test_dot(self, vectors):
        ...
        result = cas.compile_and_execute(
            lambda p1, p2: cas.Point3.from_iterable(p1).dot(cas.Point3.from_iterable(p2)), [u, v]
        )
...
class TestTransformationMatrix:
    ...
    @given(float_no_nan_no_inf(), float_no_nan_no_inf(), float_no_nan_no_inf(), unit_vector(4))
    ...
    def test_from_xyz_quat(self):
        ...
        t = cas.TransformationMatrix.from_xyz_quat(
            pos_x=1, pos_y=2, pos_z=3, quat_w=1, quat_x=0, quat_y=0, quat_z=0  # Identity quaternion
        )
    ...
    def test_quaternion_consistency(self, q):
        ...
        t = cas.TransformationMatrix.from_xyz_quat(
            pos_x=1, pos_y=2, pos_z=3, quat_w=q[3], quat_x=q[0], quat_y=q[1], quat_z=q[2]
        )
...
class TestQuaternion:
    ...
    def test_dot(self, q1, q2):
        ...
        q2 = np.array(q2)
        result = cas.compile_and_execute(
            lambda p1, p2: cas.Quaternion.from_iterable(p1).dot(cas.Quaternion.from_iterable(p2)),
            [q1, q2],
        )
...
class TestCASWrapper:
    ...
    @given(
        float_no_nan_no_inf(), float_no_nan_no_inf(), float_no_nan_no_inf(), float_no_nan_no_inf()
    )
    ...
    def test_save_division(self, f1, f2):
        assert np.isclose(
            cas.compile_and_execute(cas.save_division, [f1, f2]), f1 / f2 if f2 != 0 else 0
        )
    ...
    def test_if_greater_zero(self, condition, if_result, else_result):
        assert np.isclose(
            cas.compile_and_execute(cas.if_greater_zero, [condition, if_result, else_result]),
    ...
    def test_if_greater_eq_zero(self, condition, if_result, else_result):
        assert np.isclose(
            cas.compile_and_execute(cas.if_greater_eq_zero, [condition, if_result, else_result]),
            float(if_result if condition >= 0 else else_result),
        )
    ...
    def test_if_eq_zero(self, condition, if_result, else_result):
        assert np.isclose(
            cas.compile_and_execute(cas.if_eq_zero, [condition, if_result, else_result]),
            float(if_result if condition == 0 else else_result),
        )
    ...
    def test_if_less_eq_cases(self, a):
        ...
        assert np.isclose(
            cas.compile_and_execute(
                lambda a, default: cas.if_less_eq_cases(a, b_result_cases, default), [a, 0]
            ),
    ...
    def test_cross(self, u, v):
        assert np.allclose(cas.compile_and_execute(cas.cross, [u, v])[:3], np.cross(u, v))
    ...
    @given(
        lists_of_same_length(
            [float_no_nan_no_inf(outer_limit=1000), float_no_nan_no_inf(outer_limit=1000)],
            min_length=16,
            max_length=16,
        )
    )
    ...
    def test_velocity_limit_from_position_limit(self, acceleration, desired_result, j, step_size):
        ...
        velocity = cas.compile_and_execute(
            cas.velocity_limit_from_position_limit, [acceleration, desired_result, j, step_size]
        )
        position = j
        i = 0
    ...
    def test_distance_point_to_line_segment1(self):
        ...
        distance = cas.compile_and_execute(
            lambda a, b, c: cas.distance_point_to_line_segment(a, b, c)[0], [p, start, end]
        )
        nearest = cas.compile_and_execute(
            lambda a, b, c: cas.distance_point_to_line_segment(a, b, c)[1], [p, start, end]
        )
        assert distance == 0
        assert nearest[0] == 0
        assert nearest[1] == 0
